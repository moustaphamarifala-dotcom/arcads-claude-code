...
**Version autonome — `public/tiktok.html`.** Comme *Qui est qui ?* et *Mon Business*, le Studio TikTok existe aussi en **un seul fichier HTML** : aucune installation, aucun serveur, aucune clé. Il s'ouvre d'un double-clic ou depuis son adresse web, fonctionne sur téléphone, et pèse 67 Ko pour rester utilisable en données mobiles. Le calendrier, le diagnostic des vues et les accroches fonctionnent même hors connexion ; l'écriture des vidéos appelle Pollinations.ai directement depuis le navigateur. C'est cette version que le vendeur utilise au quotidien — la page Next.js `/tiktok` reste la version intégrée au studio.
